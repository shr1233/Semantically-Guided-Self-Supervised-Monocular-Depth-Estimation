...
class SGDepthCommon(nn.Module):
    def __init__(self, num_layers, split_pos, grad_scales=(0.9, 0.1), pretrained=False):
        ...
        self.encoders = networks.PackNetSlim01(None,None)
        ...
        self.shape_dec = (512, 128, 64, 32, 16)
...
class SGDepthDepth(nn.Module):
    def __init__(self, common, resolutions=1):
        ...
        self.dep = networks.PackNetSlim03(None,None)
    def forward(self,x):
        # x = self.decoder(*x)
...
class SGDepthSeg(nn.Module):
    def __init__(self, common):
        ...
        self.seg = networks.PackNetSlim02(None, None)
    def forward(self, x):
        # x = self.decoder(*x)
        # x = self.multires(*x[-1:])
        x = self.seg(x)
    ...
    def forward(self, batch):
        ...
        for idx, dataset in enumerate(batch):
            ...
            if self._check_purposes(dataset, 'segmentation'):
                ...
                outputs[idx]['segmentation_logits', 0] = ss
